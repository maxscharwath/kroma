...
use kroma_indexer::{definition, engine, IndexerConfig, Query};

const JSON_DEF: &str = include_str!("fixtures/synthetic-json.yml");

fn config() -> IndexerConfig {
    IndexerConfig {
        base_url: "https://json.example/".to_string(),
...
fn builds_json_request() {
    ...
    let reqs = engine::build_requests(
        &def,
        &config(),
        &Query::Text {
            query: "dune".into(),
...
fn parses_json_rows() {
    ...
    assert_eq!(
        ...
        Some("https://json.example/torrent/501")
    );
    assert_eq!(
        first.link.as_deref(),
        Some("https://json.example/download/501.torrent")
    );
