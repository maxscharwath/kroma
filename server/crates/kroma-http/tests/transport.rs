...
fn post_bytes_round_trips_arbitrary_binary() {
    ...
    let resp = Fetch::new().post_bytes(&url, "application/octet-stream", &body).unwrap();

    let request = server.join().unwrap();
    assert!(request.ends_with(&body), "every byte value must survive the transfer");
...
fn a_get_carries_its_headers_and_url_encodes_its_query() {
    ...
    assert!(request.contains("cat=2000%262010"), "a reserved char must be escaped: {request}");
...
fn get_json_surfaces_the_status_and_body_of_a_non_2xx() {
    ...
    let err = Fetch::new().get_json::<serde_json::Value>(&url).unwrap_err().to_string();
    server.join().unwrap();
...
fn a_form_post_url_encodes_every_field() {
    ...
    let resp =
        Fetch::new().post_form(&url, &[("username", "admin"), ("password", "p&ss word")]).unwrap();

    let request = text(&server.join().unwrap());
    assert!(request.starts_with("POST / HTTP/"), "{request}");
    assert!(request.to_lowercase().contains("content-type: application/x-www-form-urlencoded"));
    assert!(request.ends_with("username=admin&password=p%26ss+word"), "{request}");
...
fn a_json_post_sends_the_serialized_body() {
    ...
    Fetch::new().post_json(&url, &serde_json::json!({ "method": "torrent-get" })).unwrap();
    let request = text(&server.join().unwrap());
    assert!(request.to_lowercase().contains("content-type: application/json"), "{request}");
    assert!(request.ends_with(r#"{"method":"torrent-get"}"#), "{request}");
}
...
fn a_form_field_cannot_inject_a_second_curl_option() {
    ...
    let resp = Fetch::new().post_form(&url, &[("password", &password)]).unwrap();

    let request = text(&server.join().unwrap());
    assert!(!stolen.exists(), "an injected --output would have written this file");
    assert!(request.contains("password=p%22%0Aoutput+%3D+%22"), "{request}");
    assert_eq!(resp.text(), "Ok.");
}
...
fn a_header_value_reaches_the_server_with_its_quotes_and_backslashes() {
    let (url, server) = one_shot_server("HTTP/1.1 200 OK", "ok");
    Fetch::new().header("X-Api-Key", r#"se"cr\et"#).get(&url).unwrap();
    let request = text(&server.join().unwrap());
...
fn a_header_value_carrying_crlf_cannot_split_the_request() {
    let (url, server) = one_shot_server("HTTP/1.1 200 OK", "ok");
    Fetch::new().header("X-Api-Key", "k\r\nX-Injected: 1").get(&url).unwrap();
    let request = text(&server.join().unwrap());
...
fn post_bytes_sends_the_declared_content_type_and_body() {
    let (url, server) = one_shot_server("HTTP/1.1 201 Created", "");
    let resp = Fetch::new().post_bytes(&url, "application/octet-stream", b"\x00\x01binary").unwrap();
    let request = text(&server.join().unwrap());
    assert!(request.to_lowercase().contains("content-type: application/octet-stream"), "{request}");
...
fn a_refused_connection_reports_the_curl_exit_code() {
    let err = Fetch::new().max_time(5).get("http://127.0.0.1:1/").unwrap_err().to_string();
