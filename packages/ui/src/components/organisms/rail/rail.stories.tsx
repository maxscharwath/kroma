...
function SelectableRail({
  ...
  return (
    <Box gap={14}>
      {virtualised ? (
        // Composed: the row is a part, so the windowed one is named rather than
        // switched on by a prop.
        ...
        // Sugar: a title and tiles, which is what most rails are.
        <Rail.Root title={title} gap={gap} inset={inset}>
          {cards}
        </Rail.Root>
      )}
...
export default story({
  ...
  usage: `<Rail.Root title="Reprendre">{cards}</Rail.Root>
