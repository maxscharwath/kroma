...
The browse screens' poster grid: a library of thousands, of which only the rows near the viewport are mounted. Focus reaches the end anyway, because every row and every cell declares the slot it holds in the DATA, so a row that leaves the window and comes back lands where it was rather than at the end. On the web the wheel scrolls it too, and that moves the selection, unlike a `VirtualRail`'s wheel: the grid has no scroll position of its own, only a focused row.
