...
function load(clientList: Array<Record<string, unknown>> = []): Harness {
	...
	const clients = clientList.map((c) => ({
		...
		navigate: c.navigate ?? (async (url: string) => {
			navigated.push(url);
		}),
	}));
...
describe('push', () => {
	...
	it('collapses a retried delivery instead of stacking duplicates', async () => {
		...
		const sw = load();
		await sw.fire('push', pushEvent(payload()));
		expect(sw.shown[0].options.tag).toBe('n1');
	...
	it('still shows something when the payload is missing or unreadable', async () => {
		...
		expect(sw.shown[0].title).toBe('KROMA');
		expect(sw.shown[1].title).toBe('KROMA');
	...
	it('shows at most two action buttons', async () => {
		...
		const actions = sw.shown[0].options.actions as Array<{ action: string }>;
		...
		expect((sw.shown[0].options.data as { actions: unknown[] }).actions).toHaveLength(3);
	});
...
describe('notificationclick', () => {
	...
	it('calls the server directly for an api action without opening the app', async () => {
		...
		expect(sw.fetches[0].url).toBe('/api/requests/r1/approve');
		expect(sw.fetches[0].init).toMatchObject({ method: 'POST', credentials: 'include' });
		expect(sw.opened).toEqual([], 'an api action must not also open the app');
	});
...
describe('pushsubscriptionchange', () => {
	...
	it('re-registers the rotated subscription with the server', async () => {
		...
		expect(sw.fetches).toHaveLength(1);
		expect(sw.fetches[0].url).toBe('/api/push/subscribe');
		const body = JSON.parse(String(sw.fetches[0].init?.body));
