...
pub fn demo_data() -> ScanData {
    ...
    let items = vec![
        movie(
            ...
            vec![
                ...
                audio("aac", 2, Some("eng")),
            ],
            vec![sub(Some("eng"), "subrip"), sub(Some("fra"), "subrip")],
            &movies_lib,
            &added,
        ),
        movie(
    ...
    let shows = vec![
        Show {
            id: planet_earth,
            title: "Planet Earth II".into(),
            year: Some(2016),
            library: shows_lib.clone(),
            season_count: 1,
            episode_count: 2,
            video: None,
            added_at: added.clone(),
            metadata: None,
            progress: None,
        },
        Show {
    ...
    let libraries = vec![
        Library {
            id: movies_lib,
            name: "Films (Démo)".into(),
            kind: LibraryKind::Movies,
            path: "<demo>".into(),
            item_count: movies_count,
        },
        Library {
            id: shows_lib,
            name: "Séries (Démo)".into(),
            kind: LibraryKind::Shows,
            path: "<demo>".into(),
            item_count: shows_count,
        },
    ];
...
fn episode(
    ...
    with_demo_file(MediaItem {
        ...
        episode_end: None,
        episode_title: Some(episode_title.into()),
        rel_path: None,
        added_at: added_at.into(),
        metadata: None,
        abs_path: None,
...
mod tests {
    ...
    fn libraries_are_typed_with_item_counts() {
        ...
        let films = data
            .libraries
            .iter()
            .find(|l| l.name == "Films (Démo)")
            .unwrap();
        let series = data
            .libraries
            .iter()
            .find(|l| l.name == "Séries (Démo)")
            .unwrap();
    ...
    fn movie_audio_tracks_are_indexed_with_first_default() {
        ...
        let br = data
            .items
            .iter()
            .find(|i| i.title == "Blade Runner 2049")
            .unwrap();
    ...
    fn tracks_helper_assigns_sequential_indices() {
        let out = tracks(vec![
            audio("eac3", 6, Some("eng")),
            audio("aac", 2, Some("fra")),
        ]);
