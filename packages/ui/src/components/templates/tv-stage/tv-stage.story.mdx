...
```tsx
// Each platform shell wraps the shared app in it, once:
<TvStage>
...
<Dont>
- Nest one stage in another. The scale would compound.
- Reach for a viewport unit inside it; the canvas is fixed, so `82` already means 82.
</Dont>
