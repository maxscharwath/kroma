// Platform-independent core of the in-process libmpv engines. Must stay free of
// any `#[cfg(target_os = …)]` and of any window handle type (HWND / XID /
// NSWindow); those belong to the platform modules.

use std::sync::{Arc, Mutex};
use std::thread;

use libmpv2::events::{Event, PropertyData};
use libmpv2::{Format, Mpv, MpvInitializer, MpvStr, Result as MpvResult};
use serde_json::{json, Value};
use tauri::{AppHandle, Emitter};

/// Holds the live engine handle, empty until the platform's `init()` succeeds.
/// Each platform module re-exports it under its own name as Tauri managed state.
#[derive(Default)]
pub struct MpvSlot {
    mpv: Mutex<Option<Arc<Mpv>>>,
}

impl MpvSlot {
    pub fn set(&self, mpv: Arc<Mpv>) {
        *self.mpv.lock().unwrap() = Some(mpv);
    }

    pub fn is_active(&self) -> bool {
        self.mpv.lock().unwrap().is_some()
    }

    /// Load a URL, replacing the current file. `start` > 0 seeks during the open
    /// so playback begins at the resume point instead of buffering at 0 first.
    pub fn load(&self, url: &str, start: f64) {
        if let Some(mpv) = self.mpv.lock().unwrap().as_ref() {
            if start > 0.5 {
                let opt = format!("start={start}");
                let _ = mpv.command("loadfile", &[url, "replace", "0", &opt]);
            } else {
                let _ = mpv.command("loadfile", &[url, "replace"]);
            }
        }
    }

    /// Send a raw mpv command array (`set_property`, `seek`, `stop`, …).
    pub fn command(&self, args: &[Value]) {
        if args.is_empty() {
            return;
        }
        let mut strs: Vec<String> = args.iter().map(value_to_mpv_arg).collect();
        // The frontend speaks mpv's JSON-IPC dialect, where `set_property` is an
        // IPC-level command; in-process `mpv_command` only knows INPUT commands,
        // whose equivalent is `set`. Without the translation mpv returns -4
        // (INVALID_PARAMETER) and pause / audio-track changes silently no-op.
        if strs[0] == "set_property" {
            strs[0] = "set".to_string();
        }
        let rest: Vec<&str> = strs[1..].iter().map(String::as_str).collect();
        if let Some(mpv) = self.mpv.lock().unwrap().as_ref() {
            let _ = mpv.command(&strs[0], &rest);
        }
    }
}

/// Options every engine sets, applied after the platform's own video-output
/// options. KROMA draws its own subtitle overlay over the transparent webview,
/// so mpv must render none itself: no auto-load and no default track.
pub fn apply_common_options(init: &MpvInitializer) -> MpvResult<()> {
    init.set_property("hr-seek", "yes")?;
    init.set_property("force-seekable", "yes")?;
    init.set_property("cache", "yes")?;
    init.set_property("sub-auto", "no")?;
    init.set_property("sid", "no")?;
    init.set_property("terminal", false)?;
    // No mpv Lua: on macOS the scripts run on LuaJIT, and under the hardened
    // runtime (required for notarization) the kernel SIGKILLs the app seconds
    // into playback init with CODESIGNING / "Invalid Page" on the JIT's r-x
    // page. Every built-in script has to be named individually - `load-scripts`
    // governs only the USER scripts from the config directory, not the ones mpv
    // ships, so setting it alone leaves all of the below running.
    for option in [
        "load-scripts",
        "load-stats-overlay",
        "load-console",
        "load-auto-profiles",
        "load-select",
        "load-positioning",
        "load-commands",
        "load-context-menu",
        "osc",
        "ytdl",
    ] {
        init.set_property(option, "no")?;
    }
    Ok(())
}

/// Observe the properties the frontend `MpvEngine` reacts to. The ids (1..5)
/// are part of that protocol: keep them, and their order, stable.
pub fn observe_playback_properties(mpv: &Mpv) {
    let _ = mpv.observe_property("time-pos", Format::Double, 1);
    let _ = mpv.observe_property("duration", Format::Double, 2);
    let _ = mpv.observe_property("demuxer-cache-time", Format::Double, 3);
    let _ = mpv.observe_property("pause", Format::Flag, 4);
    let _ = mpv.observe_property("paused-for-cache", Format::Flag, 5);
}

pub fn spawn_pump(app: &AppHandle, mpv: Arc<Mpv>) {
    let app_pump = app.clone();
    thread::spawn(move || pump_events(app_pump, mpv));
}

fn pump_events(app: AppHandle, mpv: Arc<Mpv>) {
    loop {
        match mpv.wait_event(1.0) {
            Some(Ok(Event::PropertyChange { name, change, .. })) => {
                let data = match change {
                    PropertyData::Double(d) => json!(d),
                    PropertyData::Int64(i) => json!(i),
                    PropertyData::Flag(b) => json!(b),
                    PropertyData::Str(s) | PropertyData::OsdStr(s) => json!(s),
                };
                let _ = app.emit("mpv://property", json!({ "name": name, "data": data }));
            }
            Some(Ok(Event::FileLoaded)) => {
                let _ = app.emit("mpv://file-loaded", ());
                emit_track_list(&app, &mpv);
            }
            Some(Ok(Event::EndFile(reason))) => {
                let r = match reason {
                    0 => "eof",
                    4 => "error",
                    _ => "stop",
                };
                let _ = app.emit("mpv://end-file", json!({ "reason": r }));
            }
            Some(Ok(Event::Shutdown)) => break,
            _ => {}
        }
    }
}

// `track-list` has no node variant, so the list is rebuilt field by field on
// file-load and emitted as an `mpv://property`.
fn emit_track_list(app: &AppHandle, mpv: &Mpv) {
    let count = mpv.get_property::<i64>("track-list/count").unwrap_or(0);
    let mut tracks = Vec::new();
    for i in 0..count {
        let id = mpv
            .get_property::<i64>(&format!("track-list/{i}/id"))
            .unwrap_or(-1);
        let ty = mpv
            .get_property::<MpvStr>(&format!("track-list/{i}/type"))
            .map(|s| s.to_string())
            .unwrap_or_default();
        tracks.push(json!({ "id": id, "type": ty }));
    }
    let _ = app.emit(
        "mpv://property",
        json!({ "name": "track-list", "data": tracks }),
    );
}

fn value_to_mpv_arg(v: &Value) -> String {
    match v {
        Value::String(s) => s.clone(),
        Value::Bool(b) => (if *b { "yes" } else { "no" }).to_string(),
        other => other.to_string(),
    }
}
