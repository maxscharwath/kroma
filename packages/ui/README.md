<div align="center">
  ...
  <h1>@luma/ui</h1>
  <p><i>LUMA design-system React components + design tokens.</i></p>
</div>
...
```tsx
import '@luma/ui/styles.css';            // tokens + base styles (once, at the root)
import { Button, Badge, Chip, Avatar, PosterCard, Logo } from '@luma/ui';
...
`@luma/ui/styles.css` exposes the brand as CSS custom properties, so app code and
TV/web shells stay on-brand without re-declaring values:

```css
--luma-bg: #0A0A0C;        /* deep charcoal page */
--luma-text: #F4F3F0;
