...
fn a_kicked_remote_stops_being_obeyed_until_it_picks_the_set_up_again() {
    ...
    reg.attach_controller("tv-salon-01", "sock-b", "iPad", "u1", "alice", None)
        .expect("attached");

    assert_eq!(reg.may_command("tv-salon-01", "u1"), Some(true));
    reg.kick_controller("tv-salon-01", "sock-b")
        .expect("kicked");

    assert_eq!(reg.may_command("tv-salon-01", "u1"), Some(false));

    reg.attach_controller("tv-salon-01", "sock-c", "iPad", "u1", "alice", None)
        .expect("attached");
    assert_eq!(reg.may_command("tv-salon-01", "u1"), Some(true));
...
fn a_television_lists_its_remotes_and_can_let_one_go() {
    ...
    let (row, owner) = reg
        .kick_controller("tv-salon-01", "sock-b")
        .expect("kicked");
    assert_eq!(owner, "u1");
    assert_eq!(row.controllers.len(), 1);
...
fn a_television_can_only_disconnect_its_own_remotes() {
    ...
    reg.attach_controller("tv-salon-01", "sock-a", "iPhone", "u1", "alice", None)
        .expect("attached");

    assert!(reg.kick_controller("tv-chambre-02", "sock-a").is_none());
    assert_eq!(
        reg.list("u1").iter().flat_map(|r| &r.controllers).count(),
        1
    );
}
...
fn a_phone_that_reconnects_replaces_itself_instead_of_doubling() {
    let reg = Registry::new();
    announce_ok(&reg, beat("tv-salon-01", 0, None), "u1", None);
    reg.attach_controller("tv-salon-01", "sock-old", "iPhone", "u1", "alice", None)
        .expect("attached");
...
fn a_remote_for_a_receiver_that_is_gone_is_refused() {
    let reg = Registry::new();
    assert!(reg
        .attach_controller("tv-ghost-01", "sock-a", "iPhone", "u1", "alice", None)
        .is_none());
