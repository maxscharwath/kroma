...
/** Every size the design uses, smallest first. 24 is Tabler's own grid. */
const SIZES = [16, 20, DEFAULT_ICON_SIZE, 28, 36, 48];

/** The names in the fallback scene are WRONG on purpose - a typo and a name a
 *  server-installed module might invent - so they are cast rather than typed.
 *  `IconName` exists precisely to make `<Icon name="chevron-rihgt" />` fail to
 *  compile, and the one place that has to demonstrate the runtime behaviour is
 *  the one place that has to opt out of it. */
