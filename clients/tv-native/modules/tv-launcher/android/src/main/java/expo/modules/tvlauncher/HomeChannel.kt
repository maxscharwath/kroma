...
object HomeChannel {
     ...
     * this is a library module, so the app's resources are not on its R class,
     * and the banner is generated by the TV config plugin (app.json
     * `androidTVBanner`) under a name this module has no business knowing. The
     * manifest attribute is exactly the same drawable the launcher itself shows
     * for the app, which is the one that belongs on our rows. Falls back to the
     * app icon on a build that declares no banner, and to no logo at all if
     * neither resolves - a channel without a logo still works. */
