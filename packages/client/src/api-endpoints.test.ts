...
describe('delegating methods issue the expected request', () => {
  ...
  const known: Array<[string, (c: KromaClient) => unknown, string, string]> = [
    ...
    ['checkReset', (c) => c.checkReset('t'), 'GET', '/auth/reset/t'],
    ['redeemReset', (c) => c.redeemReset('t', 'c', 'p'), 'POST', '/auth/reset'],
    [
      'requestPasswordReset',
      (c) => c.requestPasswordReset('who@ever.tld'),
      'POST',
      '/auth/reset-request',
    ],
    ['checkEmailVerification', (c) => c.checkEmailVerification('t'), 'GET', '/auth/verify-email/t'],
    [
      'confirmEmailVerification',
      (c) => c.confirmEmailVerification('t'),
      'POST',
      '/auth/verify-email',
    ],
    [
      'sendEmailVerification',
      (c) => c.sendEmailVerification('u1'),
      'POST',
      '/admin/users/u1/email-verification',
    ],
  ...
  const others: Array<[string, (c: KromaClient) => unknown]> = [
    ...
    ['adminTranscodes', (c) => c.adminTranscodes()],
