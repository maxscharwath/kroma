...
// must keep the wheel inside the 66/108 safe zone, and the monochrome layer is
// the silhouette Android tints itself.
...

{
  ...
  // Sizes are exact: @react-native-tvos/config-tv copies them verbatim into the
  // TVAppIcon catalog, and Xcode/App Store submission rejects anything off by a
  // pixel or carrying alpha.
...
function markSvg(): string {
  const paths = wheelSectors(50, 50, 44, 15)
    .map((p, i) => `<path d="${p}" fill="${WHEEL_COLORS[i]}"/>`)
    .join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="6 6 88 88" role="img" aria-label="KROMA">${paths}</svg>`;
}
...
// URI, not fetched: a fetched icon behind an edge cache kept the pre-rebrand
// logo live on packages.kroma.tv long after the assets changed.
...
`;
for (const w of ['packages/synology-repo/worker', 'packages/module-registry/worker']) {
