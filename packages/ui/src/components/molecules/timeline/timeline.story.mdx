...
<Scene name="Every tone">
  Three faces: something that happened, the state the thing is in now, and where the sequence began. The origin is hollow because the rail starts there rather than passing through it.

  {story.take(({ size }) => (
    <Timeline.Root size={size}>
      {(['current', 'event', 'origin']).map((tone) => (
...
<Scene name="Glyphs">
  A glyph replaces the node where the KIND of event is worth reading at a glance.
...
<Scene name="Composed">
  Anything the two sugar props have no name for goes in as children, under them.
...
- Put a revision, a duration or a code in `detail`, and what happened in `title`.
</Do>

<Dont>
- Put a second control inside an entry: give the entry itself `onPress`.
- Reach for it to lay out a settings list. That is `<ListRow>`, which has a shell and a chevron.
</Dont>
