...
use luma_release::{Candidate, Target};
...
pub fn score_release(
    ...
    indexer: &IndexerRow,
    st: &SearchTarget,
    profile: &luma_release::Profile,
) -> ScoredReleaseView {
    let parsed = luma_release::parse_release_name(&release.title);
    ...
    let (score, breakdown, rejected) =
        match luma_release::score(&parsed, &candidate, &st.target, profile, &release.title) {
            Ok(s) => (
                Some(s.score),
                s.breakdown
                    .into_iter()
        ...
        match luma_torznab::search(&super::endpoint_of(indexer), &torznab_query, &caps) {
            Ok(found) => {
                for r in found {
                    ...
                    let p = luma_release::parse_release_name(&r.title);
