...
describe('what it recommends against', () => {
  ...
  it('says nothing rather than throwing when the server will not answer', async () => {
    ...
    const { result } = renderHook(() => useTvUpNext(client, MOVIE));
...
describe('the cards', () => {
  it("titles an episode by the episode's own name, with the runtime on the sub-line", () => {
    const client = stubClient();
    const { result } = renderHook(() => useTvUpNext(client, MOVIE, [EPISODE]));
  ...
  it('carries the first genre as the category label', () => {
    const client = stubClient();
    const { result } = renderHook(() => useTvUpNext(client, MOVIE, [MOVIE]));
    expect(result.current.data.nextEpisodes[0]?.categoryLabel).toBe('Science-Fiction');
  });
});
