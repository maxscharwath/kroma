// JS <-> ExoPlayer bridge, injected into the WebView as `__LUMA_ANDROID__`.
//
...
// pushed back by invoking the page's global `__lumaExoEvent(payload)` (see
...
class ExoBridge(
    ...
    private fun emit(payload: JSONObject) {
        val js = "window.__lumaExoEvent&&window.__lumaExoEvent($payload)"
