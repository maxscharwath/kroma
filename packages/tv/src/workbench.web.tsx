...
// Two globs: the stories and demos, then the demos as text for the code panel.
// Both written out in full and cast in place — Vite finds the call by matching the
// text `import.meta.glob(`, so hoisting the host or the options into a local stops
// it being a glob and throws at runtime.
//
// No prop docs, deliberately: they come from a Vite plugin (`propDocs` in
// clients/tv-build) that `shell.ts` does not load, so the Props tab is empty here.
