...
import { colors, radius as radii } from '../../lib/tokens';
...
/** A full on/off cycle of the caret. */
const BLINK_MS = 1100;

/** The blinking insertion point shown in on-screen-keyboard mode. It is on
 * screen for the whole time the remote is walking the on-screen keyboard, so it
 * goes through `useLoop` (a CSS keyframe on the browser TVs) rather than paying
 * a JS frame callback of its own the whole time. */
function Caret() {
  const blink = useLoop('blink', BLINK_MS);
  return (
    <Animated.View
      style={[
        { width: 2, height: 28, borderRadius: radii.sm, backgroundColor: colors.accent },
        blink,
      ]}
    />
  );
