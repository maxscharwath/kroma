...
/// `LUMA_TMDB_API_KEY` overrides it. Empty here = feature stays off until a key
...
pub struct Config {
    ...
    /// when a key is present; set `LUMA_TMDB_ENRICH=0` to disable.
    pub tmdb_enrich: bool,
    /// Public base URL of the web app (`LUMA_WEB_URL`), used to build the Quick
...
impl Config {
    ...
    pub fn from_env() -> Self {
        let host = env::var("LUMA_HOST").unwrap_or_else(|_| "0.0.0.0".to_string());

        let port = env::var("LUMA_PORT")
            .ok()
        ...
        let data_dir = env::var("LUMA_DATA_DIR")
            .map(PathBuf::from)
        ...
        let tmdb_api_key = env::var("LUMA_TMDB_API_KEY")
            .ok()
        ...
        let tmdb_language =
            env::var("LUMA_TMDB_LANGUAGE").unwrap_or_else(|_| "en-US".to_string());

        let tmdb_enrich = env::var("LUMA_TMDB_ENRICH")
            .map(|v| !matches!(v.trim(), "0" | "false" | "no" | "off"))
            .unwrap_or(true);

        let web_url = env::var("LUMA_WEB_URL")
            .ok()
    ...
    pub fn socket_addr(&self) -> SocketAddr {
        let ip: IpAddr = self.host.parse().unwrap_or_else(|_| {
            tracing::warn!(host = %self.host, "LUMA_HOST is not a valid IP; binding 0.0.0.0");
            IpAddr::from([0, 0, 0, 0])
        });
