...
import { GEN_LANGS, GEN_QUALITIES, type GenForm } from '#tv/features/playback/player/useSubtitleGen';
...
export function GeneratePanel({
  ...
  const sourceLabel = source
    ? source.label || (source.language ? (langName(t, source.language) ?? source.language.toUpperCase()) : t('player.subtitleTrack', { number: source.index + 1 }))
    : t('player.subNoSource');

  return (
    <div className="mt-3 rounded-2xl border border-[rgba(242,180,66,0.25)] bg-[rgba(20,20,28,0.6)] p-4">
      <div className="mb-3 font-display text-[16px] font-bold text-text">{t('player.subGenerate')}</div>

      {/* mode tabs (field 0) */}
      <div className={`flex gap-2 rounded-xl p-1 ${genFocus === 0 ? 'shadow-(--ring-focus-sm)' : ''}`}>
          ...
          return (
            ...
            >
              ...
              <div className="font-sans text-[11px] opacity-75">
                {m === 'transcribe' ? t('player.subModeTranscribeHint') : t('player.subModeTranslateHint')}
              </div>
            </div>
          );
        })}
      ...
      {!transcribe ? (
        <>
          ...
          <div className={`${BOX} ${genFocus === sourceIdx ? FOCUSED : RESTING} text-text`}>{sourceLabel}</div>
        </>
      ) : null}

      <div className={FIELD}>{transcribe ? t('player.subSpokenLang') : t('player.subtitles')}</div>
      <div className={`${BOX} ${genFocus === langIdx ? FOCUSED : RESTING} text-text`}>{lang.label}</div>

      {transcribe ? (
        <>
          <div className={FIELD}>{t('player.subQuality')}</div>
          <div className={`flex gap-2 rounded-xl p-1 ${genFocus === qualityIdx ? 'shadow-(--ring-focus-sm)' : ''}`}>
            {GEN_QUALITIES.map((q) => (
              <div
                key={q}
      ...
      <p className="mt-4 font-sans text-[12px] leading-relaxed text-dim">{t('player.subGenBackground')}</p>
      <div
