...
describe('resolvePluralKey', () => {
  it('picks the variant for the category count falls in', () => {
    ...
    expect(resolvePluralKey(en, en, 'en', 'n', { count: 1 })).toBe('n_one');
    expect(resolvePluralKey(en, en, 'en', 'n', { count: 3 })).toBe('n');
  });

  it('never borrows a variant from another language that happens to define one', () => {
    const en = { n: '{count} items' };
    const fr = { n: '{count} objets', n_one: '{count} objet' };

    expect(resolvePluralKey(en, fr, 'en', 'n', { count: 1 })).toBe('n');
  });

  it('does use the default catalog when the locale does not know the key at all', () => {
    const fr = { n: '{count} objets', n_one: '{count} objet' };

    expect(resolvePluralKey(NONE, fr, 'fr', 'n', { count: 1 })).toBe('n_one');
  });

  it('prefers an explicit zero variant over the category', () => {
    const en = { n: '{count} items', n_one: '{count} item', n_zero: 'Nothing yet' };

    expect(resolvePluralKey(en, en, 'en', 'n', { count: 0 })).toBe('n_zero');
    expect(resolvePluralKey(en, en, 'en', 'n', { count: 1 })).toBe('n_one');
  });
  ...
  it('lets a French zero take the zero variant ahead of its one category', () => {
    ...
    expect(resolvePluralKey(fr, fr, 'fr', 'n', { count: 0 })).toBe('n_zero');
  });

  it('falls back to the other variant when the category has no entry', () => {
    const en = { apples_other: '{count} apples' };

    expect(resolvePluralKey(NONE, en, 'ru', 'apples', { count: 3 })).toBe('apples_other');
  });

  it('pluralises on a variable that is not called count', () => {
    const en = { moved: '{files} files', moved_files_one: '{files} file' };

    expect(resolvePluralKey(en, en, 'en', 'moved', { files: 1 })).toBe('moved_files_one');
    expect(resolvePluralKey(en, en, 'en', 'moved', { files: 4 })).toBe('moved');
  });
  ...
  it('gives count priority so a second numeric variable cannot take over', () => {
    ...
    expect(resolvePluralKey(en, en, 'en', 'both', { count: 1, files: 1 })).toBe('both_one');
  });

  it('ignores a non-numeric variable', () => {
    const en = { hi: 'Hello {name}', hi_one: 'nope' };

    expect(resolvePluralKey(en, en, 'en', 'hi', { name: 'Ana' })).toBe('hi');
  });

  it('returns the bare key when nothing is declared', () => {
    const en = { hi: 'Hello' };

    expect(resolvePluralKey(en, en, 'en', 'hi', { count: 2 })).toBe('hi');
  });
});
