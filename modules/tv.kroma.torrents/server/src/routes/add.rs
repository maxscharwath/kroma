...
//! itself.
//!
//! It does NOT queue anything. The indexer path hands the manual-add flow a link
//! and a release name; this hands it the same two things read out of a file, so
//! from there the flow is identical: analyse the files, pin the title, add. The
//! bytes are kept, keyed by info hash, so when the grab does arrive the engine
//! starts from the file rather than resolving the magnet we minted from it.
...
pub async fn inspect<S: HostStorage + Clone + Send + Sync + 'static>(
    ...
    // What the release name says it is for, so the flow opens on the right shape
    // instead of asking the operator to restate it.
