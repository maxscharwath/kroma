...
//! and per-run log lines. All timestamps are epoch milliseconds.
...
/// Mark every run still flagged `running` as `failed`; called once at startup, since
/// in-process run state does not survive a restart and such a row would otherwise
/// stay `running` forever. Returns how many were fixed.
pub fn reconcile_running_runs(pool: &Pool) -> Result<usize> {
    let conn = pool.get()?;
    let now = kroma_primitives::now_ms();
    // Collect the stranded ids first, to leave an explanatory line in each run's own
    // log rather than have it stop mid-stream with no reason.
    let ids: Vec<String> = {
...
mod tests {
    ...
    fn pool() -> TempPool {
        crate::testing::temp_pool("jobs")
    }

    #[test]
    fn reconcile_flips_only_running_to_failed() {
        let p = pool();
        insert_job_run(&p, "r1", "cache.cleanup", "manual", 1).unwrap();
        insert_job_run(&p, "r2", "cache.cleanup", "manual", 1).unwrap();
        finish_job_run(&p, "r2", "success", 2, None).unwrap();
