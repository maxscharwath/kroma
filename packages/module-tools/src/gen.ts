...
//
// The `id` MUST be reverse-DNS (e.g. "tv.kroma.notes"); it is the runtime
// join key and is sanitized into the crate / package names.

import { mkdirSync, readdirSync, readFileSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fenced, frontmatter, REVERSE_DNS, slug } from './format';
...
function genModule(): string {
  return `use kroma_module_sdk::EmbeddedModule;
...
const rustHeader = (file: string) =>
  `//! @generated by packages/module-tools/src/gen.ts from modules/${file} - edit that file, then run \`bun run modules:gen\`.\n`;
...
const generated: Gen[] = [];
// Guards against two sources resolving to the same runtime id or crate name
// (e.g. `tv.kroma.a-b` and `tv.kroma.a.b` both slug to `a-b`).
...
for (const file of files) {
  ...
  if (!REVERSE_DNS.test(id)) {
    throw new Error(`${file}: id "${id}" must be reverse-DNS (e.g. tv.kroma.notes)`);
  }
  ...
  const crate = `kroma-${name}`;
  const pkg = `@kroma/${name}`;
  ...
  // The ui imports these into the module's `locales`.
  ...
  write(
    ...
    `# @generated by packages/module-tools/src/gen.ts from modules/${file}
# Its own cargo workspace, like every module: nothing above modules/<id> is a
# workspace root, so package metadata and dependency specs are explicit here.
[workspace]
...
# release + panic=abort: a sidecar aborts and the supervisor respawns it, which
# drops the unwinding tables for a smaller binary at no speed cost.
...
// Path deps for every crate the roster wires in: the single-file codegen
// crates plus the first-party roster crates (manifest-only modules have none).
const crateDeps = [
  ...roster.filter((r) => r.crate).map((r) => ({ crate: r.crate as string, id: r.id })),
  ...generated.map((g) => ({ crate: g.crate, id: g.id })),
];
// A MODULE const per crate module, or an inline EmbeddedModule (from the
// packaged module.json + optional icon) for the crate-less manifest-only
// ones. Roster first, then the codegen modules.
const manifestRegs = [
  ...roster.map((r) => {
    if (!r.manifestOnly) {
...
write(
  ...
  `# @generated by packages/module-tools/src/gen.ts - do not edit.
[package]
...
console.log(
  `rosters: ${generated.length} module(s) -> kroma-modules-generated + @kroma/modules-generated`,
);
