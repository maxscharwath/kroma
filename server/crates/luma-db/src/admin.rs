...
/// playback registry.
pub fn admin_users(pool: &Pool) -> Result<Vec<luma_domain::AdminUser>> {
    let conn = pool.get()?;
    ...
    for row in rows {
        let (u, last_seen) = row?;
        out.push(luma_domain::AdminUser {
            role: luma_domain::role_label(&u.permissions).to_string(),
            id: u.id,
            email: u.email,
            username: u.username,
...
pub fn top_users(pool: &Pool, since: i64, limit: usize) -> Result<Vec<luma_domain::TopUser>> {
    let conn = pool.get()?;
    let mut stmt = conn.prepare(
...
pub fn history_since(pool: &Pool, since: i64) -> Result<Vec<luma_domain::HistoryRow>> {
    let conn = pool.get()?;
    let mut stmt = conn.prepare(
        "SELECT ended_at,kind,watched_ms FROM play_history WHERE ended_at >= ?1 ORDER BY ended_at",
    )?;
    let rows = stmt.query_map(params![since], |r| {
        Ok(luma_domain::HistoryRow {
            ended_at: r.get(0)?,
...
pub fn library_stats(pool: &Pool) -> Result<Vec<luma_domain::LibraryStat>> {
    let conn = pool.get()?;
    let mut stmt = conn.prepare(
