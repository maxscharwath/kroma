...
pub fn upsert_job_schedule(pool: &Pool, key: &str, schedule: Option<&str>, enabled: bool) -> Result<()> {
    ...
    conn.execute(
        ...
        params![key, schedule, enabled as i64, crate::services::jobs::now_ms()],
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn pool() -> Pool {
        ...
        let path = std::env::temp_dir().join(format!("luma-jobs-{}-{n}.db", std::process::id()));
