...
const TEST = 'packages/ui/audit/dom-report.test.tsx';
...
const KNOWN = new Set(['--tree', '--smells', '--quiet', '--help', '-h']);
...
const tree = flags.has('--tree') || only.length > 0;
...
const run = spawnSync(VITEST, ['run', '--project', 'audit', TEST, '--reporter=dot'], {
  cwd: ROOT,
  env,
