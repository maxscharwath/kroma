...
<Scene name="Every ending at once">
  The three things a row can be saying, together: in flight, signed in, and gone. Only the first is still pressable, since the other two have spent their beacons, and each of them has taken the check string off the row, since a set that is answering is no longer a set that has to be told apart.
