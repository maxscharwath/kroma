...
import { loadLocalePref, saveLocalePref, setSessionStorage } from '../../core/session';
import { MemStorage } from '../../core/session/storage.fixture';
...
describe('forgetAccount', () => {
  ...
  it('scopes the removal to a server when one is given', () => {
    saveSession(session('u1', 'http://a'));
    saveSession(session('u1', 'http://b'));
    forgetAccount(UserId.parse('u1'), 'http://a');
