...
//!
//! This reads a file format that belongs to another crate, which is a coupling
//! worth naming. It fails soft in every direction: anything unreadable or
//! unexpected is left exactly as it was, and the session starts as it would
//! have.
...
// A persisted torrent is restorable without the network only if its metadata is
// on disk. A zero-length file is what librqbit leaves when it never resolved
// one, and it is the case that hangs.
