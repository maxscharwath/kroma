...
import { TYPES } from './field.fixtures';
...
```tsx
<Field.Root label="Server address">
...
<Scene name="With error" args={{ error: 'The server did not answer.', hint: '' }}>
  The error takes the hint's place and paints the entry's border, rather than adding a second line under it.
</Scene>

<Scene name="Password" args={{ label: 'Password', type: 'password', value: 'hunter2', hint: '', icon: 'lock' }}>
  The reveal eye is part of the `type`, not something a screen wires.
</Scene>

<Scene name="No label row" args={{
  label: 'Search',
  hideLabel: true,
  icon: 'search',
  type: 'search',
...
<Scene name="Every type">
  One prop decides the keyboard, the autofill hint and the masking. Flip `type` in the panel to compare; these are the six a client actually needs.
...
<Dont>
- Stack a hint under an error. The error takes the hint's place.
- Drop the `label` to hide it. `hideLabel` stops it being drawn and keeps it as the accessible name; an unnamed input is a bug on every platform.
- Reach for `keyboardType` directly. That is what `type` derives.
- Set `size` on a part: the Root owns the shell, so a form stays one family.
</Dont>
