...
//! the JSON below with structs of its own, and drives an aria2 JSON-RPC daemon.
//!
...
//! two aria2 instances, and the credentials belong to the row, not here.
...
fn engine<S: HostCtx>(_host: &S, client: &Client) -> Aria2 {
...
mod tests {
    ...
    fn unreachable_client() -> serde_json::Value {
        ...
        json!({ "url": "http://127.0.0.1:1/jsonrpc", "password": "secret" })
    }
    ...
    async fn every_method_is_mounted_where_the_consumer_calls_it() {
        ...
        // with no compile error anywhere. `reannounce` is excluded: aria2 has no
        // tracker re-announce, so it is a no-op that succeeds without reaching
        // the daemon.
        for method in ["test", "add", "pause", "resume", "remove"] {
    ...
    #[tokio::test]
    async fn reannounce_is_a_no_op_that_succeeds_without_a_daemon() {
        let (status, answer) = call(
        ...
        assert!(answer["Ok"].is_null(), "reannounce should return Ok(null): {answer}");
    }
    ...
    fn every_call_carries_the_client_it_is_for() {
        let body = json!({
            "client": { "url": "http://nas:6800/jsonrpc", "password": "secret" },
            "args": { "client_ref": "abc" },
        });

        let call: Call = serde_json::from_value(body).unwrap();

        assert_eq!(call.client.url, "http://nas:6800/jsonrpc");
        assert_eq!(call.client.password, "secret");
    ...
    fn an_add_arrives_with_a_link_a_directory_and_a_label() {
        let body = json!({
            "client": { "url": "http://nas:6800/jsonrpc" },
            "args": {
    ...
    fn a_call_with_no_args_at_all_is_accepted() {
        ...
        let call: Call =
            serde_json::from_value(json!({ "client": { "url": "http://nas:6800/jsonrpc" } })).unwrap();
