...
const s = styles({ stateLabel: { flex: true, textAlign: 'right' } });
  ...
  return (
    <>
      <Table.Row onPress={onOpen}>
        ...
        <Table.Cell row wrap align="center" gap={6} justify="flex-end">
        ...
        <Table.Cell row align="center" gap={8} justify="flex-end">
          <Text variant="meta" color={stalled ? 'danger' : 'textDim'} style={s.stateLabel}>
...
export function InstalledList({
  ...
  return (
    <Table.Root columns={[{}, { width: 260, from: 'md' }, { width: 132 }, { width: 44 }]}>
      {shown.map((m) => (
        <InstalledRow
          key={m.id}
          m={m}
