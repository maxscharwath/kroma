...
pub async fn get_llm(
    ...
    let default_id = settings::default_provider(s)
        .map(|p| p.id)
        .unwrap_or_default();
...
pub async fn save_llm(
    ...
    let providers: Vec<LlmProvider> = body
        ...
        .map(|p| LlmProvider {
            ...
            name: p.name,
            provider: if p.provider.trim().is_empty() {
                "openai".into()
            } else {
                p.provider
            },
...
pub async fn llm_models(
    ...
    let result = tokio::task::spawn_blocking(move || {
        crate::infra::llm::list_models(&provider, &base_url, &api_key)
    })
    .await
