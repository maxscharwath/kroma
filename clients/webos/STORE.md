...
**Developer Mode, on any set you can touch.** Install the *Developer Mode* app from
...
**Alpha Test** (Seller Lounge → `Applications > Alpha Test`) is the closest thing
...
stabilization". A plain new seller account does not have it, so ask through the
...
Mandatory, and **initialised on every version bump**, so refill it each release.
