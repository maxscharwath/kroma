...
//!
//! The bytes are kept, because the embedded engine can start from them without
//! waiting on the DHT to resolve a magnet. Everything else downstream speaks
//! `magnet_or_url`, so the file also yields the magnet that stands for it: an
//! external engine, which cannot be handed bytes across the wire, gets a link it
//! can act on.
...
// Percent-encoding for the one field a magnet carries free text in. Written out
// rather than pulled in: the unreserved set is four rules long.
...
/// Keeps the bytes beside the ledger row that points at them.
