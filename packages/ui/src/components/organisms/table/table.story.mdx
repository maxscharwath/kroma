...
<Scene name="Columns that hold">
  ...
  The second table is narrower than those columns want: a table never scrolls sideways, so a `flex` column shrinks to its `min` and truncates what it holds, and a column that cannot fit a narrow screen says `from: 'md'` and leaves rather than pushing the last column out of the frame.
...
- Give a `flex` column a `min`, so a long title stops shrinking rather than squeezing to nothing.
</Do>
