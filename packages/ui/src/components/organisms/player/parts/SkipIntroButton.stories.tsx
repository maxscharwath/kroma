...
export default story({
  ...
  usage: `<SkipIntroButton visible={inIntro} focused={nav.at === 'skip'} onSkip={jumpPastIntro} />`,
  guidelines: {
    do: [
      'Drive `visible` from the chapter data, and let it unmount itself when there is nothing to skip.',
    ],
    dont: ["Don't keep it on screen after the intro has passed - it becomes a mystery button."],
  },
  matrix: false,
  ...
  render: ({ visible, focused }) => (
    ...
    // the pill's 214pt inset is measured from the bottom of it.
    <Box aspect={16 / 9} minH={280} bg="surface2" radius="lg" overflow="hidden">
      <SkipIntroButton visible={visible} focused={focused} onSkip={() => {}} />
    </Box>
  ),
  scenes: [
