import { GEN_LANGS, GEN_QUALITIES, type GenQuality, type LumaClient, type MediaItem, type SubCapabilities, type SubtitleGeneration } from '@luma/core';
...
export function useSubtitleGen(
  ...
  const [form, setForm] = useState<GenForm>({ mode: 'transcribe', langIndex: 0, quality: 'balanced', sourceIndex: 0 });
  ...
  const { generations, cancel, refresh } = useSubtitleGenerations(client, item.id, { active: true, onComplete });
  ...
  const start = useCallback(() => {
    ...
    if (form.mode === 'transcribe') {
      void client
        .generateSubtitle(item.id, { mode: 'transcribe', lang: lang.label, spokenLang: lang.code, quality: form.quality })
        .catch(() => undefined);
    } else {
  ...
  const cycleLang = useCallback((dir: number) => setForm((f) => ({ ...f, langIndex: mod(f.langIndex + dir, GEN_LANGS.length) })), []);
  const cycleQuality = useCallback(
    (dir: number) => setForm((f) => ({ ...f, quality: GEN_QUALITIES[mod(GEN_QUALITIES.indexOf(f.quality) + dir, GEN_QUALITIES.length)]! })),
    [],
  );
  const cycleSource = useCallback(
    (dir: number) => setForm((f) => ({ ...f, sourceIndex: sources.length ? mod(f.sourceIndex + dir, sources.length) : 0 })),
    [sources.length],
  );
  ...
  const fields = useMemo<GenField[]>(() => {
    const modeField: GenField = { key: 'mode', onLeft: () => setMode('transcribe'), onRight: () => setMode('translate') };
    const langField: GenField = { key: 'lang', onLeft: () => cycleLang(-1), onRight: () => cycleLang(1) };
    const startField: GenField = { key: 'start', onEnter: start, closeOnEnter: true };
    if (form.mode === 'transcribe') {
      return [modeField, langField, { key: 'quality', onLeft: () => cycleQuality(-1), onRight: () => cycleQuality(1) }, startField];
    }
    return [modeField, { key: 'source', onLeft: () => cycleSource(-1), onRight: () => cycleSource(1) }, langField, startField];
