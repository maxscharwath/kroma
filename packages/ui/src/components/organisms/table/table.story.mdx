...
<Scene name="Columns that hold">
  The same four columns at two widths. State, Added and Size state a `width` and keep it to the pixel; Title takes what is left, and would stop at its `min` of 160 rather than squeezing to nothing. Size sits at the end of its cell, where a number is read against the ones above it. Added names `from: 'md'` and is gone below that breakpoint, which follows the WINDOW rather than this canvas, so narrow the browser to watch it leave the header and every row at once.

  A table narrower than its fixed columns plus its minimums has nowhere to put the difference and is clipped by its own frame today. Scrolling it sideways is the next thing this component grows.
  ...
  {story.take(({ variant }) => (
    <Box gap={24}>
      ...
      <Box gap={8}>
        ...
        <Columned variant={variant} width={530} />
      </Box>
    </Box>
  ))}
</Scene>
