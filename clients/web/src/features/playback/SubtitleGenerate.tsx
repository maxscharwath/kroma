        ...
        <div className="mb-4">
          ...
          {sources.length ? (
            <select
              ...
              className="w-full rounded-lg border border-white/12 bg-white/4 px-3 py-2.5 text-[14px] text-text outline-none focus:border-accent/50"
            >
              {sources.map((s) => (
                <option key={s.index} value={s.index}>
                  {s.label || (s.language ?? '').toUpperCase() || t('player.subtitleTrack', { number: s.index + 1 })}
                </option>
              ))}
            </select>
          ) : (
