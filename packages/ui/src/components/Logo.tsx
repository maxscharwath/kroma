export interface LogoProps {
  ...
  size?: number;
  /** Hide the "LUMA" wordmark, showing only the aperture mark. */
  markOnly?: boolean;
}
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: size * 0.42 }}>
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <circle cx="12" cy="12" r="9" stroke="var(--luma-accent)" strokeWidth="2.4" />
        <circle cx="12" cy="12" r="3.2" fill="var(--luma-accent)" />
      </svg>
      {markOnly ? null : (
        <span
          style={{
