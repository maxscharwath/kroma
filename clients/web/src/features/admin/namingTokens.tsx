    ...
    >
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
          <label className="mb-4 flex items-center gap-2 text-[12px] font-semibold text-dim">
            {t('naming.separator')}
            <select
              value={separator}
              onChange={(e) => setSeparator(e.target.value)}
              className="rounded-lg border border-border-strong bg-[#0F0F13] px-2.5 py-1.5 text-[13px] font-semibold text-text outline-none focus:border-accent/60"
            >
              {SEPARATORS.map((s) => (
