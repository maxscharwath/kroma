//! Portable backup: dump the *non-reproducible* identity state (accounts,
//! settings, watch history, resume positions, invites, cron overrides) to a
//! JSON document and restore it on another server.
//!
...
/// AI sections, sessions, job run history) is regenerated and left out.
const TABLES: &[&str] = &["users", "settings", "invites", "play_history", "job_schedules", "progress"];
...
pub fn export_portable(pool: &Pool) -> Result<BackupDoc> {
    ...
    Ok(BackupDoc { version: VERSION, exported_at: now_or_blank(), tables })
}

/// Restore a [`BackupDoc`] into the database, replacing rows by primary key.
/// Runs in one transaction with foreign keys disabled (see module docs).
/// Returns `(table, rows_written)` pairs in restore order.
pub fn import_portable(pool: &Pool, doc: &BackupDoc) -> Result<Vec<(String, usize)>> {
    ...
    let result = restore_all(&mut conn, doc);
...
fn restore_all(conn: &mut rusqlite::Connection, doc: &BackupDoc) -> Result<Vec<(String, usize)>> {
...
mod tests {
    ...
    fn roundtrip_restores_state_and_keeps_orphan_progress() {
        ...
        import_portable(&dst, &doc).unwrap();
