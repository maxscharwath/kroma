...
// (.github/assets/kroma-lockup-source.svg). Run: bun packages/core/scripts/gen-brand-assets.ts
import { mkdir } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';
import sharp from 'sharp';
import { wheelSectors } from '../src/brand';
...
// One file per site, served as a static asset. A site that also runs a worker
// still gets it from the asset handler, which answers before the worker does -
// so there is no second, inlined copy for the worker to serve.
const MARK = markSvg();
for (const site of ['clients/web', 'apps/modules', 'apps/packages']) {
