...
/// A [`HostCtx`] the bridge tests share.
///
/// Every bridge handler takes a host it never actually uses (the `&dyn HostCtx`
/// argument is dropped from the wire and re-supplied on the provider side), so
/// each module used to define its own identical 15-method stub. One copy: the
/// duplication was real, and so was the coverage noise from four sets of methods
/// nobody calls.
#[cfg(test)]
