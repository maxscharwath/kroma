...
fn combine(ledger: Option<String>, artifact_done: bool, assume_done: bool) -> String {
    crate::services::pipeline::elements::resolve_status(
        ledger.as_deref(),
        artifact_done,
        assume_done,
    )
...
pub async fn item_processing(
    ...
    let out = blocking(move || {
        ...
        let mut ts = vec![t(
            "probe",
            combine(
                crate::db::pipeline::worst_status(db, "probe", &files)?,
                crate::db::item_probed(db, &id)?,
                false,
            ),
        )];
        if matches!(item.kind, Kind::Movie | Kind::Video) {
            ts.push(t(
                "metadata",
                combine(
                    crate::db::pipeline::task_status(db, "metadata", &id)?,
            ...
            ts.push(t(
                "storyboard",
                combine(
                    crate::db::pipeline::task_status(db, "storyboard", &id)?,
                    state.storyboard.is_cached(&item),
                    true,
                ),
            ));
            ts.push(t(
                "subtitles",
                combine(
                    crate::db::pipeline::task_status(db, "subtitles", &id)?,
                    false,
                    true,
                ),
            ));
            ts.push(t(
                "embed",
                combine(
                    crate::db::pipeline::task_status(db, "embed", &id)?,
                    crate::db::has_vector(db, &id)?,
                    false,
                ),
            ));
        } else {
            ...
            let season_ledger = match (item.show_id.as_deref(), item.season) {
                (Some(s), Some(n)) => {
                    crate::db::pipeline::task_status(db, "markers", &format!("{s}#{n}"))?
                }
                _ => None,
            };
            ts.push(t(
                "markers",
                combine(season_ledger, crate::db::has_markers(db, &id)?, true),
            ));
        }
...
pub async fn show_processing(
    ...
    let out = blocking(move || {
        ...
        let eps: Vec<&crate::model::MediaItem> = detail
            .seasons
            .iter()
            .flat_map(|s| s.episodes.iter())
            .collect();
        let ep_ids: Vec<String> = eps.iter().map(|e| e.id.clone()).collect();
        let season_ids: Vec<String> = detail
            .seasons
            .iter()
        ...
        let any_markers = ep_ids
            .iter()
            .any(|e| crate::db::has_markers(db, e).unwrap_or(false));
        let treatments = vec![
            t(
                "probe",
                combine(
                    crate::db::pipeline::worst_status(db, "probe", &file_ids)?,
                    all_probed,
                    false,
                ),
            ),
            ...
            t(
                "storyboard",
                combine(
                    crate::db::pipeline::worst_status(db, "storyboard", &ep_ids)?,
                    all_cached,
                    true,
                ),
            ),
            t(
                "subtitles",
                combine(
                    crate::db::pipeline::worst_status(db, "subtitles", &ep_ids)?,
                    false,
                    true,
                ),
            ),
            t(
                "markers",
                combine(
                    crate::db::pipeline::worst_status(db, "markers", &season_ids)?,
                    any_markers,
                    true,
                ),
            ),
