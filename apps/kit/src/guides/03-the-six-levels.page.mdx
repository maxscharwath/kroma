...
The split earns its keep for one reason. You can read a level without reading the
app.
...
| 6. Pages | not here | A template filled with real data, covered below. |
...
<Figure row caption="Level 3: a callout and a readout. Neither adds a value of its own. A callout is an icon and two lines of text, a readout is an overline and a line under it. Both are here because the design names the arrangement, not because the pieces were hard to write.">
...
screens, `clients/web/src` and `clients/mobile/src` for the browser and phone
ones, and `apps/*/src` for the websites. A kit that shipped pages would be
...
before gzip. What buys that is `"sideEffects": false` in the package manifest, the
one thing that lets a bundler drop a barrel's untouched re-exports. Delete it and
door 1 keeps the whole kit: a single `Text` off the flat barrel goes from 33 KB
gzipped to 166 KB. Nothing in this package runs at import time, so the field is
