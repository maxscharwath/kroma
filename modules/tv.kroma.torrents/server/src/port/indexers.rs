...
pub fn fetch_torrent(
    ...
    let Some(resolve) = resolve(host, TORRENT_FETCH) else { return Ok(None) };
    call(&resolve, &format!("{TORRENT_FETCH}/fetch"), &json!({ "indexer_id": indexer_id, "url": url }))
}
...
pub fn names(host: &dyn HostCtx) -> Vec<IndexerName> {
    let Some(resolve) = resolve(host, INDEXER_DB) else { return Vec::new() };
    call::<_, Vec<IndexerName>>(&resolve, &format!("{INDEXER_DB}/list"), &json!({})).unwrap_or_default()
}
...
mod tests {
    ...
    fn an_absent_indexer_module_degrades_rather_than_failing() {
        ...
        assert_eq!(fetch_torrent(&host, "idx-1", "http://t/f.torrent").unwrap(), None);
