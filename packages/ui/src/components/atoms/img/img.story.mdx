...
```tsx
<Img src={client.media.artwork.resolve(item.posterUrl)} background={tintGradient(item.tint)} radius={13} />
```
...
<Scene name="Ratio already matches" args={{ src: stillArt(0) }}>
  A 16:9 still in the 16:9 frame. Here `cover` and `contain` are the SAME picture, and `position` does nothing, because there is nothing to crop. Art the server sized for the box is the case worth aiming for.
</Scene>
