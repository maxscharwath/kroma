...
//! the JSON below with structs of its own, and drives a Transmission daemon.
//!
//! Every method carries the client it is for. This process is stateless about
//! which of the operator's configured clients it is serving.
...
/// rename here breaks it in another process, which the tests below pin.
...
fn engine(client: &Client) -> Transmission {
    Transmission::new(&crate::ClientDef {
        kind: KIND.to_string(),
...
async fn test<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    port_reply(move || engine(&call.client).test()).await
}
...
async fn add<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    port_reply(move || {
        ...
        engine(&call.client).add(&req)
    })
    .await
}

// Not the `Result` envelope: `None` is a torrent the daemon has forgotten, which
...
async fn status<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    let found = tokio::task::spawn_blocking(move || {
        engine(&call.client).status(&call.args.client_ref).ok().flatten().map(wire)
    })
    .await
    .ok()
    .flatten();
...
async fn pause<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    port_reply(move || engine(&call.client).pause(&call.args.client_ref)).await
}
...
async fn resume<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    port_reply(move || engine(&call.client).resume(&call.args.client_ref)).await
}
...
async fn reannounce<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    port_reply(move || engine(&call.client).reannounce(&call.args.client_ref)).await
}
...
async fn remove<S: HostCtx + Clone + Send + Sync + 'static>(
    ...
    port_reply(move || engine(&call.client).remove(&call.args.client_ref, call.args.delete_data))
        .await
}
...
mod tests {
    ...
    fn every_call_carries_the_client_it_is_for() {
        let body = json!({
            "client": { "url": "http://nas:9091", "username": "u", "password": "p" },
            "args": { "client_ref": "abc" },
        });

        let call: Call = serde_json::from_value(body).unwrap();

        assert_eq!(call.client.url, "http://nas:9091");
    ...
    fn a_call_with_no_args_at_all_is_accepted() {
        ...
        let call: Call =
            serde_json::from_value(json!({ "client": { "url": "http://nas:9091" } })).unwrap();
    ...
    fn a_status_answers_the_keys_the_consumer_reads() {
        let json = serde_json::to_value(wire(crate::TorrentStatus {
            client_ref: "abc".into(),
    ...
    #[test]
    fn a_paused_torrent_crosses_as_the_lowercase_word() {
        let status = wire(crate::TorrentStatus {
            client_ref: "abc".into(),
            name: String::new(),
            info_hash: None,
            progress: 0.1,
            state: crate::TorrentState::Paused,
            down_bps: 0,
            up_bps: 0,
            peers: 0,
            peers_seen: 0,
