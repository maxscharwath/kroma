...
this and where is the media and nothing more, because a first-time operator should reach a
...
attack surface, since possession of the machine is the authentication. It never reads existing
...
Background work, meaning scans, metadata refreshes, transcodes and module tasks, is visible, honest
...
- **Being counted.** The owner may let this install be counted among the KROMA servers
  ...
  titles and no exact counts. The payload is printed in full in its own job log before it is sent, so the
...
the server entirely, stand up a new one, restore one backup, and be where they were, with the same
accounts, the same history and the same settings, without re-teaching it anything.
...
- **Server updates** restart the server and therefore interrupt streams, so the owner is
