...
fn every_15_minutes() {
    ...
    assert_eq!(
        next("*/15 * * * *", from),
        datetime!(2026-06-29 10:15:00 UTC)
    );
    let from = datetime!(2026-06-29 10:55:30 UTC);
    assert_eq!(
        next("*/15 * * * *", from),
        datetime!(2026-06-29 11:00:00 UTC)
    );
}
...
fn weekday_names_and_or_rule() {
    ...
    assert_eq!(
        next("0 9 * * mon", from),
        datetime!(2026-06-29 09:00:00 UTC)
    );
...
fn seconds_field() {
    ...
    assert_eq!(
        next("30 0 10 * * *", from),
        datetime!(2026-06-29 10:00:30 UTC)
    );
}
...
fn ranges_and_lists() {
    ...
    assert_eq!(
        next("0 9-17 * * *", from),
        datetime!(2026-06-29 12:00:00 UTC)
    );
    let from = datetime!(2026-06-29 11:00:00 UTC);
    assert_eq!(
        next("0,30 * * * *", from),
        datetime!(2026-06-29 11:30:00 UTC)
    );
}
...
fn open_ended_step_form() {
    ...
    assert_eq!(
        next("5/10 * * * *", from),
        datetime!(2026-06-29 10:05:00 UTC)
    );
