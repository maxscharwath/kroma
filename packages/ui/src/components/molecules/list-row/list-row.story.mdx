...
A focusable menu or settings row, composed of named parts. This shape had been written three times before landing here: the television's profile menu, the out-of-session settings, and the admin's lists.

```tsx
<ListRow.Root icon="settings" onPress={open}>
...
A row that goes somewhere rather than acting in place is the link itself, and its parts go inside it. See `asChild` on **Focusable**.

```tsx
<ListRow.Root icon="settings" asChild>
  <RouteLink to="/account/playback">
    <ListRow.Label>Playback</ListRow.Label>
  </RouteLink>
</ListRow.Root>
```
...
<Scene name="Media at the head">
  The head of the row takes an avatar, a poster thumb or a status dot as readily as a glyph.
...
<Scene name="A group">
  One card for the whole list: the group carries the surface and declares the size its members take.
...
<Dont>
- Nest another Focusable inside the row; the whole row is the control.
- Put a control in `<ListRow.Trailing>` where a face will do. A `<SwitchFace>` reads as a switch without stealing the row's press.
</Dont>
