...
// jsdom has no media stack, so the timeline is driven through plain stubs.
function stubMedia() {
...
describe('KromaIntro (film path)', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    stubMedia();
    // Sound-first autoplay is blocked (as in Chrome); the muted retry succeeds.
    play.mockReset();
  ...
  it('ignores a video error that lands after a skip (no second intro)', async () => {
    ...
    // The CSS fallback renders no <video>.
