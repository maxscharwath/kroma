...
export function TvGrid({ label, cards }: { label: string; cards: GridCard[] }) {
  const [count, sentinel] = useGrowingCount(cards.length, GRID_STEP);
  return (
    <div className="scrollbar-none min-h-0 flex-1 overflow-y-auto px-16 pt-6 pb-[72px]">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(188px,1fr))] gap-x-6 gap-y-8">
        {cards.slice(0, count).map((c) => (
          <TvPoster key={c.id} title={c.title} badge={c.badge} poster={c.poster} colors={c.colors} onClick={c.onClick} />
        ))}
      </div>
