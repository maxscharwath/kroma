#!/usr/bin/env bash
# LUMA Tizen on-device live-dev: serve the TV shell over the LAN with Vite HMR so a
...
#
# The LUMA server must be running and reachable on the LAN (it binds 0.0.0.0:4040):
#
...
# the baked-in address and the HMR address match. Override with LUMA_TV_HOST.
HOST_IP="${LUMA_TV_HOST:-$("$(dirname "$0")/lan-ip.sh")}"
if [ -z "$HOST_IP" ]; then
  echo "dev-device: could not detect a LAN IP." >&2
  echo "  Set it explicitly:  LUMA_TV_HOST=192.168.1.20 bun run dev:tizen:device" >&2
  exit 1
fi

echo "TV dev server → http://$HOST_IP:5174/   (seeds API → http://$HOST_IP:4040)"
echo "Needs: LUMA server running (bun run server:watch) + dev shell installed (make dev-shell)."

export LUMA_TV_DEVICE=1
export LUMA_TV_HOST="$HOST_IP"
...
export VITE_LUMA_SERVER="${VITE_LUMA_SERVER:-http://$HOST_IP:4040}"

exec bun run --filter '@luma/tizen' dev
