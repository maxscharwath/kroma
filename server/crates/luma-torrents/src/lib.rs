...
/// Resolve a client definition to its engine (mirrors the LLM `provider_for`).
/// `rqbit` is the embedded engine handle owned by the caller (None when it
/// failed to start or is not compiled in); `state_dir` hosts per-client
/// scratch (qBittorrent cookie jars).
pub fn client_for(
    def: &ClientDef,
    ...
    match def.kind.as_str() {
        "rqbit" => match rqbit {
            ...
            None if RQBIT_COMPILED => bail!("embedded engine not started"),
            None => bail!("embedded engine not compiled (torrent-rqbit feature off)"),
        },
        "transmission" => Ok(Box::new(Transmission::new(def))),
        "qbittorrent" => Ok(Box::new(QBittorrent::new(def, cookie_jar_path(state_dir, def)))),
        other => bail!("unknown download client kind {other:?}"),
    }
}
