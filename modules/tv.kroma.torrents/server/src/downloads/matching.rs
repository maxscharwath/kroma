...
//!
//! A grab that came from a request already knows; a `.torrent` an operator
//! dropped on the queue knows only its own release name. This reads that name
//! the way the acquisition stack reads an indexer result, asks the host what
//! could match, and takes the answer only when it is confident enough. A row an
//! operator has corrected is never re-matched.
...
pub fn shape_of(release_title: &str) -> ReleaseShape {
    ...
    // An episode range ("S01E01-E03") is several episodes in one torrent.
...
impl DownloadManager {
    ...
    pub fn auto_link(&self, host: &dyn HostCtx, id: &str) {
        ...
        drop(conn);
        if row.tmdb_id != 0 || row.match_source.as_deref() == Some(MANUAL) {
            return;
        }
        ...
        let Some(best) = candidates
            ...
            // Marked tried, so the background pass moves on instead of asking
            // the provider about this row forever. A correction overwrites it.
        ...
        let link = DownloadLink {
            ...
            // The provider's year is the authority once we trust the match; the
            // parsed one was only ever a hint used to find it.
